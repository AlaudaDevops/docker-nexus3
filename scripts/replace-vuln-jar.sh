...
replace_dependency() {
    ...
    if [[ -f "$old_jar_path" ]]; then
        ...
        ls -la $work_dir/BOOT-INF/lib/ | grep $jar_name || true
    ...
    # Download new version from Maven Central
    ...
    cd $work_dir
    jar -uf ${origin_jar} BOOT-INF/lib/$jar_name-$new_version.jar
    # Remove old version
    log "Removing old version jar: $old_jar_path"
    zip -d ${origin_jar} BOOT-INF/lib/$jar_name-$old_version.jar
    cd -
...
for replace_jar_info in $(echo $REPLACE_JAR_INFO | tr ',' '\n'); do
    ...
    replace_dependency "$ORIGINAL_JAR"  "$replace_jar_path" "$old_version" "$new_version" "$EXECUTION_DIR" "$local_jar"
done


cd "$TEMP_DIR"
jar uf "$ORIGINAL_JAR" BOOT-INF/layers.idx
jar uf "$ORIGINAL_JAR" BOOT-INF/classpath.idx
cd -
